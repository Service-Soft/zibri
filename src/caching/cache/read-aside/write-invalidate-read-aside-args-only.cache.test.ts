...
import { WriteInvalidateReadAsideArgsOnlyCache } from './write-invalidate-read-aside-args-only.cache';
...
@Cache()
class MainCache extends WriteInvalidateReadAsideArgsOnlyCache<string, string, 'Write Invalidate Read Aside Args Only Main Cache'> {
    constructor(
        ...
        super('Write Invalidate Read Aside Args Only Main Cache', new InMemoryCacheStore(), 'all');
    }
}

@Cache()
class AffectedCache extends WriteInvalidateReadAsideArgsOnlyCache<string, string, 'Write Invalidate Read Aside Args Only Affected Cache'> {
    constructor(
        @Inject(ZIBRI_DI_TOKENS.LOGGER)
        protected readonly logger: LoggerInterface,
        @Inject(ZIBRI_DI_TOKENS.METRICS_SERVICE)
        ...
        super('Write Invalidate Read Aside Args Only Affected Cache', new InMemoryCacheStore(), ['invalidate-me']);
    }
}
...
describe('WriteInvalidateReadAsideArgsOnlyCache', () => {
    ...
    it('wrapWrite deletes the arg-derived key and invalidates matching caches', async () => {
