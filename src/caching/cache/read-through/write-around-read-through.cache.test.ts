...
import { WriteAroundReadThroughCache } from './write-around-read-through.cache';
...
@Cache()
class MainCache extends WriteAroundReadThroughCache<string, string, 'Write Around Read Through Main Cache'> {
    constructor(
        ...
        super('Write Around Read Through Main Cache', new InMemoryCacheStore(), 'all');
    }
}

@Cache()
class AffectedCache extends WriteAroundReadThroughCache<string, string, 'Write Around Read Through Affected Cache'> {
    constructor(
        @Inject(ZIBRI_DI_TOKENS.LOGGER)
        protected readonly logger: LoggerInterface,
        @Inject(ZIBRI_DI_TOKENS.METRICS_SERVICE)
        ...
        super('Write Around Read Through Affected Cache', new InMemoryCacheStore(), ['invalidate-me']);
    }
}
...
describe('WriteAroundReadThroughCache', () => {
    ...
    it('wrapWrite calls fn, invalidates tags, and bypasses its own cache', async () => {
    ...
    it('reads still go through the cache (inherited from ReadThroughCache)', async () => {
        const cache: MainCache = inject(MainCache);

        // eslint-disable-next-line typescript/typedef
        const fn = jest.fn((id: string): string => `computed:${id}`);
        // eslint-disable-next-line typescript/typedef
        const wrapped = cache.wrap(fn, (id) => `key:${id}`);

        await expect(wrapped('a')).resolves.toBe('computed:a');
        expect((await cache.store.get('key:a'))?.value).toBe('computed:a');
