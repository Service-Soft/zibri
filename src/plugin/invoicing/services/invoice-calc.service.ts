...
export class InvoiceCalcService implements InvoiceCalcServiceInterface<Invoice> {
    ...
    getItemTotalPriceBeforeTax(item: InvoiceItem): BigNumber {
        return NumberUtilities.multiply(item.amount, item.price);
    }
    ...
    getItemTotalPriceAfterTax(item: InvoiceItem): BigNumber {
        ...
        const taxMultiplier: BigNumber = NumberUtilities.add(1, NumberUtilities.divide(item.vat.rate, 100));
        return NumberUtilities.multiply(itemTotalBeforeTax, taxMultiplier);
    }

    // eslint-disable-next-line jsdoc/require-jsdoc
    getItemTotalTax(item: InvoiceItem): BigNumber {
        const itemTotalBeforeTax: BigNumber = this.getItemTotalPriceBeforeTax(item);
        return NumberUtilities.multiply(itemTotalBeforeTax, NumberUtilities.divide(item.vat.rate, 100));
    }

    // eslint-disable-next-line jsdoc/require-jsdoc
    getTotalBeforeTax<Invoice extends BaseInvoice>(invoice: Invoice): BigNumber {
        let res: BigNumber = NumberUtilities.new(0);
        for (const item of invoice.items) {
            res = NumberUtilities.add(res, this.getItemTotalPriceBeforeTax(item));
        }
        return res.lt(0) ? NumberUtilities.new(0) : res;
    }
    ...
    getTotalAfterTax<Invoice extends BaseInvoice>(invoice: Invoice): BigNumber {
        ...
        return NumberUtilities.add(totalBeforeTax, totalTax);
    }
    ...
    getTotalTax<Invoice extends BaseInvoice>(invoice: Invoice): BigNumber {
        let res: BigNumber = NumberUtilities.new(0);
        for (const item of invoice.items) {
            res = NumberUtilities.add(res, this.getItemTotalTax(item));
        }
        return res;
    }
